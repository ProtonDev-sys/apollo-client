...
The Apollo server emits incremental search snapshots. The client consumes those snapshots directly, so a fast provider can populate the interface without waiting for the slowest provider.
