...
const WINDOWS_PORTABLE_BUILDER = "scripts/build-windows-portable.js";
...
function collectElectronRuntimeErrors(projectRoot) {
  ...
  const preparePackage = String(packageJson.scripts?.["prepare:package"] || "");
  const coreWindowsBuild = String(packageJson.scripts?.["build:win"] || "");
  const socialWindowsBuild = String(packageJson.scripts?.["build:win:social"] || "");
  const portableWindowsBuild = String(packageJson.scripts?.["build:win:portable"] || "");
  ...
  if (!/rmSync\((['"])native-bin\1/.test(preparePackage)) {
    errors.push("Package preparation must remove stale optional native resources before recreating native-bin.");
  }
  if (!coreWindowsBuild.includes("electron-builder")
      || coreWindowsBuild.includes("build:discord-social-helper")) {
    errors.push("The default Windows build must package the compact Electron core without the optional Discord Social SDK.");
  }
  if (!socialWindowsBuild.includes("prepare:package")
      || !socialWindowsBuild.includes("build:discord-social-helper")
      || !socialWindowsBuild.includes("electron-builder")) {
    errors.push("The optional Discord Social build must remain an explicit clean Windows build target.");
  }
  if (!portableWindowsBuild.includes(WINDOWS_PORTABLE_BUILDER)) {
    errors.push(`The portable Windows build must run ${WINDOWS_PORTABLE_BUILDER}.`);
  }
